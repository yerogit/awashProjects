...
APP.service('Session', function() {
	this.create = function(data) {
		...
		console.log("HERE ARE THE ROLES: " + this.userRoles);
	};
...
APP
		.service(
				'AuthSharedService',
				function($rootScope, $http, $resource, authService, Session,
						$filter) {
					return {
						...
						getAccount : function() {
							...
							$http.get('security/account').then(
									function(response) {
										console.log(response);
										authService
												.loginConfirmed(response.data);
									});
						},
...
APP
		.service(
				...
				function($rootScope, $location, $http, AuthSharedService,
						Session, USER_ROLES, $q, $timeout, Service) {
						...
						run : function() {
											...
											function(event, next) {
												...
												if (next.originalPath === "/login"
													...
													$rootScope
														...
														&& !AuthSharedService
																.isAuthorized(next.access.authorizedRights)) {
							...
							$rootScope
									.$on(
											...
											function(event, data) {
												console
														.log('login confirmed start: '
																+ data.Role);
												$rootScope.loadingAccount = false;
												console.log($location.path());

												var nextLocation = ($rootScope.requestedUrl
														&& $rootScope.requestedUrl !== '/login' ? $rootScope.requestedUrl
														: "/home");
							...
							// Call when the 401 response is returned by the
							// server
							$rootScope.$on('event:auth-loginRequired',
									function(event, data) {
										if ($rootScope.loadingAccount
												&& data.status !== 401) {
											$rootScope.requestedUrl = $location
													.path()
							...
							// Call when the 403 response is returned by the
							// server
									...
									.$on(
											...
											function(event, data) {
												if (data.status === -1) {
													Service
															.errorToast(
																	"Unable to connect to remote servers. Please check your network.",
																	null,
																	{
																		progressBar : true,
																		timeOut : 10000
